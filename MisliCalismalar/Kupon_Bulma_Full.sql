...
Begin
	...
	set @IntegrationId=(select KuponNumarasi from Kamer where sayi=@Sayi)
	set @MemberId=32679757
	set @KuponAdet=(select Kupon_Adeti from Kamer where sayi=@Sayi)
	...
	select  @Sayi KuponNumarasi,convert(date,ECM.CreateDate) Kupontarihi,
		...
		,Case when ECM.CouponType=2 then  --'Sistemli'
										convert(Nvarchar(10),(select  ECM_.Combinations - (sum(convert(int,ECD_.StandartFlag))) 
											from [10.0.1.109].[ATGBet].[dbo].[EventCouponMasters] ECM_
												inner join [10.0.1.109].[ATGBet].[dbo].[EventCouponDetails] ECD_ on ECD_.CouponId=Ecm_.CouponId
											where ECM_.IntegrationId= @IntegrationId
											group by ECM_.Combinations)) + ' ' + 'li Sistem'
		   Else 'Kombine'
		   End SistemliKombine
		,ECM.Price Tutar
